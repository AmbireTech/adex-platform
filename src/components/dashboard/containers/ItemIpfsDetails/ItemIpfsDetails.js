...
export class ItemIpfsDetails extends Component {
	constructor(props) {
		...
		this.state = {
			details: null,
			itemIpfsUrl: Item.getIpfsMetaUrl(
				this.props.itemIpfs,
	...
	itemDetails = ({ details = {}, t } = {}) => {
		return (
			<Grid container spacing={1}>
				<PropRow left={t('PROP_FULLNAME')} right={details.fullName} />
				<PropRow
					left={t('PROP_OWNER')}
					right={
						<Anchor
							target='_blank'
							href={process.env.ETH_SCAN_ADDR_HOST + details.owner}
						>
							{' '}
							{details.owner || '-'}{' '}
						</Anchor>
				...
				<PropRow
					left={t('PROP_SIZE')}
					right={t(AdSizesByValue[details.size].label, {
						args: AdSizesByValue[details.size].labelArgs,
					})}
				/>
				<PropRow
					left={t('PROP_AD_URL')}
					right={
						<Anchor target='_blank' href={details.ad_url}>
							{' '}
							{details.ad_url || '-'}{' '}
						</Anchor>
					}
					...
					<PropRow
						...
						right={
							<Img
								...
								className={''}
								src={
									ItemModel.getImgUrl(details.img, process.env.IPFS_GATEWAY) ||
									''
								}
	...
	reportDetails = ({ report = {}, t } = {}) => {
		return (
			<Grid container spacing={1}>
				<PropRow left={t('PROP_BIDID')} right={report.bidId} />
				<PropRow
					left={t('PROP_ADVERTISER')}
					right={
						<Anchor
							target='_blank'
							href={process.env.ETH_SCAN_ADDR_HOST + report.advertiser}
						>
							{' '}
							{report.advertiser || '-'}{' '}
						</Anchor>
				...
				<PropRow
					...
					right={
						<Anchor
							target='_blank'
							href={process.env.ETH_SCAN_ADDR_HOST + report.publisher}
						>
							{' '}
							{report.publisher || '-'}{' '}
						</Anchor>
				...
				/>
				<PropRow left={t('PROP_ALLCLICKS')} right={report.allClicks} />
				<PropRow
			...
			<ContentBox>
				...
				<Grid container spacing={1} style={{ marginBottom: 8 }}>
					<PropRow
						left={t('IPFS')}
						right={
							<Anchor target='_blank' href={this.state.itemIpfsUrl}>
								{' '}
								{this.props.itemIpfs}{' '}
							</Anchor>
						}
				...
				{!!this.props.spinner || !details ? (
					...
					<ContentBody>
						{this.props.detailsType === 'item' ? (
							<this.itemDetails details={details} t={t} />
						) : null}
						{this.props.detailsType === 'report' ? (
							<this.reportDetails report={details} t={t} />
						) : null}
					</ContentBody>
				)}
			</ContentBox>
