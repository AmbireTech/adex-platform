...
import { PublisherReceiptTpl } from './ReceiptTemplates'
						...
						{receipts.length > 0 && (
							<Box ref={invoice} className={classnames(classes.a4)}>
								{receipts.map(campaignId => (
									<PublisherReceiptTpl date={date} key={campaignId} />
								))}
							</Box>
						)}
					</Box>
